...
def complete_impact_data(df_impact_grid):
    ...
    df_complete = all_combinations.merge(df_impact_grid, 
    ...
    df_impact_complete = pd.concat([
    df_typhoons_nodmg_exploded, df_complete
...
def create_dynamic_features(cell_size, rain_dir):
    ...
    df_impact_grid = df_impact_grid.rename(
        {'id':'grid_point_id',
         'Year':'typhoon_year'}, axis=1)

    df_impact_complete = complete_impact_data(df_impact_grid)

    # Merge features
    df_wind_impact = df_windfield.drop('geometry', axis=1).merge(df_impact_complete)
    df_wind_impact['typhoon'] = df_wind_impact['typhoon_name'] + df_wind_impact['typhoon_year'].astype(str)
...
if __name__ == "__main__":
    ...
    # Cell size
    cell_size = 0.5
    df_dynamic = create_dynamic_features(cell_size=cell_size, rain_dir=rain_dir)
    # Save to blob
    blob_name = f"{PROJECT_PREFIX}/GRID_CELL_SIZE/{cell_size}/dynamic_features.csv"
