...
def create_rainfall_dataset(event_metadata):
    # Setting directories
    input_dir = (
        Path(os.getenv("STORM_DATA_DIR"))
        / "analysis_hti/05_realtime_forecasts/rainfall/input"
    )

    output_dir = (
        Path(os.getenv("STORM_DATA_DIR"))
        / "analysis_hti/05_realtime_forecasts/rainfall/output"
    )
...
def load_rainfall_data(date=today):
    ...
    ) / "analysis_hti/05_realtime_forecasts/rainfall/output/NOMADS/{}".format(
        date
    )
    # We are just interested in the spatial mean of the accmulated rainfall by grid
    df_rain = pd.read_csv(rain_dir / "rainfall_data_rw_mean.csv")
    return df_rain
